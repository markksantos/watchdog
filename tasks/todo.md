...
- [x] App icon asset (Watchdog/Resources/AppIcon.icns, generated by scripts/make_icon.py)
- [x] Unit tests — WatchdogTests target, 26 tests (trial, schedule, gating, persistence)
- [x] StoreKit Testing config (Watchdog.storekit: monthly/annual + 7-day annual trial)
- [x] Zero build warnings (debug + release); removed dead asset catalog + Preview Content
- [x] Git hygiene: untracked .build/, build/, .DS_Store, Xcode user state (public repo)
- [x] GitHub Actions CI (.github/workflows/ci.yml: build + release + test + bundle verify)
- [x] Landing page migrated Netlify → Vercel (landing/vercel.json)
- [x] README updated to match reality

## Blocked on Mark (Apple Developer account / product decision)
- [ ] Create live StoreKit products in App Store Connect (IDs already wired)
- [ ] Distribution decision: Mac App Store vs notarized DMG
- [ ] Xcode archive flow + Developer ID code-signing + notarization (DMG path)
